...
const templatingWithMustache = () => {
    ...
    characters.forEach(value => {
        ...
        arr.push(Mustache.render(musTemplate, value));
    });
    return arr;
};
...
const totalCharacters = (arr) => {
    ...
    characters.forEach(value => {
        if(value.children.length > 0){
          ...
          numArr.push(value.children);
        }
            numArr.push(value.name);
            numArr.push(value.spouse);
            numArr.push(value.house);
        });
...
const houseSize = (arr) => {
    ...
    arr.forEach(val => {
        ...
        let obj = {
            house: val.house,
            members: 1+ i + val.children.length
        }
        sizes.push(obj);
    });
