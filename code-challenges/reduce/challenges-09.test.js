...
function getCurrentEvents(request, response) {
  // Solution code here...
  return mapCurrentEvents()
      .then(eventsData => {
          response.ststus(200).json(eventsData);
      });
}
const mapCurrentEvents = () => {
  // Solution code here...
  let eventArr = currentEvents.news.map(value => {
      let newEvent = new Event(value);
      return newEvent;
  });
  return eventArr;
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let lengthArr = arr.reduce((acc, value, idx) => {
      acc = acc + 1;
      return acc;
  }, 0);
  return lengthArr;
...
const returnNames = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((acc, value, idx) => {
      acc[idx] = value.name;
      return acc;
  }, []);
  return newArr
...
const reversedString = (str) => {
  ...
  let revers = string.reduce((acc, value, idx) => {
      return value + acc;
  }, '');
  return revers;
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let allChildren = arr.reduce((acc, value, idx) => {
      if (value.children) {
...
const calculateAverage = (arr) => {
  // Solution code here...
  let avg = arr.reduce((acc, val, idx) => {
      return acc += val;
  }, 0);
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let primeNum = arr.reduce((acc, val, idx) => {
      let count = 0;
        if(isPrime(val)){
          count++
        }
