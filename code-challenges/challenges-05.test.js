...
const templateWithJQuery = () => {
  ...
  for (let index = 0; index < starWarsPeople.length; index++) {
    ...
    template.find('h2').text(starWarsPeople[index].name);
    template.find('h3').text(starWarsPeople[index].height);
    template.find('p').text(starWarsPeople[index].eye_color);
...
const listFoods = (recipe) => {
  ...
  for (let index = 0; index < ingredients.length; index++) {
    let tempStr = ingredients[index].split(" ");
    let regex = /,/g;
    result.push(tempStr.slice(2).toString().replace(regex, ' '));
}
  return result;
};
